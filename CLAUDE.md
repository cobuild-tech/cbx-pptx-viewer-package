...
`app/` is deliberately excluded from the root `workspaces` array in `package.json` so that by default it exercises the actual published package like any external consumer, not local source — after changing `packages/core` you must `npm run build`, bump the version and republish before `app/` picks the change up.

**To develop `app/` against local source instead, set `USE_LOCAL_PACKAGE=true`** (in `app/.env` or the shell): `app/vite.config.ts` then aliases `@cobuildx.ai/office-viewer` and `.../react` to `packages/core/src`, hot-reloading on every edit with no build or publish. That is the only way to exercise the React layer (`react/PptxViewer.tsx`, `EditorToolbar`, `ParaControls`, `ToolbarUi`) before a release — `demo/` is vanilla TS and never renders it. Run it with `npm --prefix app run dev` after setting the variable, and unset it to check the published package again.

`demo/` is aliased to local source and hot-reloads unconditionally.
...
npm run dev                    # app/ dev server (npm --prefix app run dev)
                               # USE_LOCAL_PACKAGE=true aliases it to packages/core source
...
- PPTX **paragraph formatting** (`viewer.toggleList('bullet'|'number')`, `indentSelection(±1)`, `applyParaFormat({ align, lineSpacingPct, spaceBeforePt, spaceAfterPt, level, list })`): bulleted and numbered list toggles, indent/outdent through the nine levels (Tab / Shift+Tab inside text), alignment, and line/paragraph spacing, written into the paragraph's own `<a:pPr>` by `pptx/edit/paraProps.ts`. One default style each — `•` in Arial, `arabicPeriod` numbers; there is no style picker. A command works from a bare caret (the paragraph the caret sits in *is* the target) and applies to every paragraph the selection touches. `onParaSelectionChange` reports the state for a toolbar, omitting any property the selected paragraphs disagree about.
...
- **A bullet takes its look from the text it marks, not the box.** DrawingML's defaults are `buClrTx`/`buSzTx`/`buFontTx` — colour, size and typeface come from the paragraph's *first run* — so `bulletSpan` reads them off that run, with an explicit `<a:buClr>`/`<a:buSzPct>`/`<a:buFont>` overriding and `buSzPct` *scaling* the run's size. Styling the glyph from the paragraph's ambient CSS instead drew 36pt red text with a small black bullet.
- **Every "this is not content" marker is written as an attribute** (`setAttribute('contenteditable', 'false')`), and reconciliation checks the attribute *and* the property. They reflect each other in a browser but not in jsdom, and a bullet read back as content gets typed into the deck.
- **A format marker must never span two paragraphs.** `applyFormatToSelection` wraps the selection in a marker span, and `Range.extractContents()` *moves* nodes — so one span across a paragraph boundary pulls the second paragraph's content into the first, merging paragraphs and dropping the text at the seam. The range is clamped to each paragraph and wrapped once per paragraph instead.
- **Focus moving into the toolbar is not leaving the text.** Opening a `<select>` blurs the box; treating that as "done editing" committed and closed the session, so the `change` that followed applied to nothing. Toolbar chrome is marked `data-cbx-toolbar` (`EDIT_ATTR.chrome`) and the focusout handler ignores it. Likewise a *detached* node is ignored — a commit re-renders, and the removed box fires focusout on the way out.
- **The toolbar must be told when nothing is in effect.** Reporting only while the caret is in text leaves every toggle lit for text the user has since left, which reads as a button stuck on. `reportFormatState` reports empty formats when the selection is not inside the live slide, and is pushed after a commit as well as on `selectionchange` — restoring the same range does not always raise that event.
- **Never mix the `border` shorthand with `borderColor` in React inline styles.** React does not reliably clear the longhand it set last render when the next one only carries the shorthand, so a pressed button kept its outline after its formatting stopped applying. Toolbar state lives in classes and `aria-pressed` (`react/ToolbarUi.tsx`), not in inline styles.
- **A commit used to eject the user from the text box.** `EditContext` decides where contentEditable goes by *object identity*, and a commit rebuilds the slide's model, so after a formatting commit the stored body matched nothing: the box came back read-only with the caret gone, while `isEditingText` still said true. `commitBody` now re-adopts the body through `Deck.bodyForNode` — the XML node survives a rebuild where the model does not — and puts the caret back by paragraph index + character offset (`readCaretAddress`/`restoreCaretAddress`), counting only editable text so a bullet glyph does not shift it.
- **Turning a bullet off means *stating* `<a:buNone/>`, not deleting `<a:buChar>`.** A body placeholder inherits its bullet from the master, so a deletion just re-inherits it and the button appears not to work. The same logic makes "on" always explicit. `<a:pPr>` therefore grows on paragraphs the user has touched, and inheritance for that one property is deliberately severed — as in PowerPoint.
- **`<a:pPr>`'s `bu*` families are exclusive choice groups**, so a write clears the whole group (`buNone`/`buAutoNum`/`buChar`, and `buFontTx`/`buFont` alongside) rather than replacing one sibling. Its children are also a sequence: `insertInOrder` (now in `oxml/xml.ts`, shared with the `<a:rPr>` and worksheet writers) places them by `PPR_ORDER`. A bullet write must also set `buFont` — a master stating `Wingdings` would otherwise draw `•` as whatever that font has at the code point.
- **A list only gets a hanging indent where none is inherited.** The renderer draws the bullet in a gutter only when the resolved `indent` is negative, so a plain text box needs `marL`/`indent` written; a placeholder must not have them written, or a later level change stops moving the text. Changing level shifts an *existing* explicit `marL` by one step and otherwise writes nothing.
